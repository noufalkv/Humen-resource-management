                              ...
                              <thead>
                                ...
                                <tbody id="equipmnet_info">
                                    <tr>
                                        ...
                                        <?php //echo form_dropdown_multiple('equipment[]',$equip,$equip->equipment_id,'class="form-control equipment" required id="equipment_id_1" ') ?>
                                        
                                            <select name="equipment[]" class="form-control equipment" multiple id="equipment_id_1">  
                                                         <?php foreach($equip as $category):?>                                              
                                                         <?php //$selected = in_array($category->equipment_id,$vendorcat) ? " selected " : null;?>
                                                               <option value="<?=$category->equipment_id?>"
                                                                      <?//=$selected?> ><?=$category->equipment_name?>
                                                                 </option>                              
                                                             <?php endforeach?>
                                             </select>
                                        ...
                                        <!-- <input type="text" name="equipment[]" class="form-control equipment" required="" onkeypress="asset_autocomplete(1);" id="equipment_id_1"/>-->
                                        <!-- <input type="hidden" class="autocomplete_hidden_value" name="equipment_id[]" id="Hiddenid"/>
  ...
  /*  function addasset(divName){
        ...
        if (count == limits)  {
            ...
            var tabin="equipment_id_"+count;
             ...
             newdiv.innerHTML ='<td><input name="equipment[]" class="form-control equipment" required="" id="equipment_id_1" ><input type="hidden" class="autocomplete_hidden_value" name="equipment_id[]" id="Hiddenid"/><input type="hidden" class="sl" value="'+ count +'"></td><td><input type="text" name="dates[]" class="form-control datepicker" value=""></div></td><td><button style="text-align: right;" class="btn btn-danger red" type="button" value="<?php //echo display("delete")?>" onclick="deleteRow(this)"><i class="fa fa-close"></i></button></td>';
