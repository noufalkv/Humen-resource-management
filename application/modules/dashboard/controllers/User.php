...
class User extends MX_Controller {
	...
	{ 
		...
		$data['branch'] = $this->user_model->branch_list();
		
		
		/*-----------------------------------*/
		$this->form_validation->set_rules('firstname', display('firstname'),'required|max_length[50]');
			$this->form_validation->set_rules('brand', display('brand'),'required');
   ...
   //      }
	...
	public function employeeList(){
		...
		$datas = $this->db->select('employee_id,first_name,last_name')->from('employee_history')->where('branch_id',$branch_id)->get()->result_array();		
		$info_arr = array();
