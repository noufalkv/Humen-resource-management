...
class Auth extends MX_Controller {
	...
	{  
		...
		#-------------------------------------#n
		$this->form_validation->set_rules('branch', display('branch1'), 'required');
	...
	/*	$this->form_validation->set_rules(
		...
		if ( $this->form_validation->run())
		{
			...
			$financial_year = $this->auth_model->financial_year();						
		if($user->num_rows() > 0) {
			...
			if($user->row()->is_admin == 2){
              ...
              $employee_info = $this->db->select('employee_id,first_name,last_name,is_super_visor')->from('employee_history')->where('email',$user->row()->email)->get()->row();
				...
				$this->session->set_flashdata('exception', display('incorrect_email_or_password_or_branch'));
				redirect('login');
			} 
		    ...
		    $data['branch'] 	 = $this->auth_model->branch_list();
