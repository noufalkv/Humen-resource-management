...
public function holiday_delete($id = null){
    ...
    public function manageleave()
	{
            ...
            $this->db->select('count(DISTINCT(ap.leave_appl_id)) as leave_appl_id,ap.*,p.employee_id,p.first_name,p.last_name, type.leave_type')  ; 
            $this->db->from('leave_apply ap');
            $this->db->join('employee_history p', 'ap.employee_id = p.employee_id', 'left');
            $this->db->join('leave_type as type', 'type.leave_type_id = ap.leave_type_id', 'left');
            if($this->session->userdata('branch_id')!= 1)
            {
            $this->db->where('branch_id',$this->session->userdata('branch_id'));
            }
            $this->db->group_by('ap.leave_appl_id');
            $this->db->order_by('ap.leave_appl_id', 'desc');
           $query=   $this->db->get();
           return  $query->result();
    }
