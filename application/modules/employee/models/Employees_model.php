...
public function insert_employee($data = array())
	...
	public function emplyee_history_delete($id = null)
	{
		...
		if ($this->db->affected_rows()) { 
			...
			$this->db->where('employee_id',$id)
			->delete('employee_nominee_details');
	...
	{
		return $this->db->select('d.*,p.*')	
			->from('position as p')
			->join('department as d','p.dept_id=d.dept_id')
			->order_by('p.pos_id', 'desc')
			->get()
			->result();
	}
	...
	public function pos_updateForm($id){
               ...
               $this->db->select('d.*,p.*');
                   $this->db->from('position as p');
		            $this->db->join('department as d','p.dept_id=d.dept_id');
		            $this->db->where('pos_id',$id);
		            $query = $this->db->get();
	...
	public function employee_updateForm_new($id){
        
		$this->db->where('employee_id',$id);
        $query = $this->db->get('employee_education');
        $data['education'] =  $query->row();
		
		$this->db->where('employee_id',$id);
        $query = $this->db->get('employee_experience');
        $data['experience'] =  $query->row();		
		
		$this->db->where('employee_id',$id);
        $query = $this->db->get('employee_bank');
        $data['bank'] =  $query->row();
		
		$this->db->where('employee_id',$id);
        $query = $this->db->get('employee_nominee_details');
        $data['nominee_details'] =  $query->result();	
		return $data;
    }
	...
	public function files($id){
		$this->db->select('e.*,c.type as type,');
        $this->db->join('category as c','e.parent_id=c.cat_id');
        $this->db->where('e.file_emp',$id);
	...
	public function insert_new_post($data_arr)
	{
		$this->db->insert('employee_experience', $data_arr['experience']);
		$this->db->insert('employee_education', $data_arr['education']);
		$this->db->insert('employee_bank', $data_arr['bank']);
		$this->db->insert('employee_gross_salary', $data_arr['gross']);
		$nominee_name = array();
		$nominee_relation = array();
		$nominee_dob = array();
		$nominee_occupation = array();
		
		$nominee_name = $data_arr['nominee']['nominee_name'];
		$nominee_relation = $data_arr['nominee']['nominee_relation'];
		$nominee_dob = $data_arr['nominee']['nominee_dob'];
		$nominee_occupation = $data_arr['nominee']['nominee_occupation'];
		
		foreach($nominee_name as $key=>$value )
		{
			$row = [
			'employee_id' => $data_arr['employee_id'],
			'name' => $value,
			'relation' => $nominee_relation[$key],
			'date_of_birth' => $nominee_dob[$key],
			'occupation' => $nominee_occupation[$key]
			];
			
			$this->db->insert('employee_nominee_details', $row);
		}
	...
	public function update_employee_new($data = array())
	{
		$this->db->where('emp_exp_id', $data["emp_exp_id"])->update("employee_experience", $data['experience']); 
		$this->db->where('emp_edu_id', $data["emp_edu_id"])->update("employee_education", $data['education']);
		$this->db->where('emp_bank_id', $data["emp_bank_id"])->update("employee_bank", $data['bank']); 
		$gross = $this->db->select('*')   
            ->from('employee_gross_salary')
            ->order_by('added_date', 'desc')
            ->get()
            ->row();
		
		if($gross->rate != $data["gross"]["rate"])
		{
			$this->db->insert('employee_gross_salary', $data['gross']);
		}
		$emp_nom_id = array();
		$nominee_name = array();
		$nominee_relation = array();
		...
		$emp_nom_id = $data['nominee']['emp_nom_id'];
		$nominee_name = $data['nominee']['nominee_name'];
		$nominee_relation = $data['nominee']['nominee_relation'];
		$nominee_dob = $data['nominee']['nominee_dob'];
		$nominee_occupation = $data['nominee']['nominee_occupation'];
		foreach($nominee_name as $key=>$value )
		{
			$row = [
			...
			$this->db->where('emp_nom_id', $emp_nom_id[$key])->update("employee_nominee_details", $row); 
		}
	  return true;
	}

public function insert_paid_leave($employee_id,$financial_year)
	{
		$data = [
		...
		$this->db->insert('employee_paid_leave', $data );
