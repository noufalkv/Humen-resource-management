...
<div class="panel panel-default" style="float: left;width: 100%;">
			...
			<?php if( $this->session->userdata('branch_id') == 1) { ?>
               <div class="panel-body" style="padding: 15px;float: left;">	
				<?php echo form_open('employee/Employees/manageemployee',array('class' => 'form-inline', 'id' => 'validate'));?>		
				...
				<?php } ?>
        </div><div class="row">    <!--  table area -->    <div class="col-sm-12">      
     ...
     <th><?php echo display('hire_date')?></th> 
      ...
      <!--  <td><?php// echo $row->state; ?></td>  -->                         
        ...
        <td><?php $supervisor = $this->db->select('first_name,last_name')->from('employee_history')->where('employee_id',$row->super_visor_id)->get()->row();               
        ...
        <a href="<?php echo base_url("employee/Employees/delete_employhistory/$row->employee_id") ?>" class="btn btn-xs btn-danger" onclick="return confirm('<?php echo display('are_you_sure') ?>') "><i class="fa fa-trash"></i></a>                                         <a href="<?php echo base_url("employee/Employees/cv/$row->employee_id");?>" class="btn btn-default"><i class="fa fa-user"></i></a>                                                                           </td>                                </tr>                                <?php $sl++; ?>                            <?php } ?>                         <?php } ?>                     </tbody>                </table>  <!-- /.table-responsive -->            </div>        </div>    </div></div>